...
def swamee_jain_f(reynolds: float, roughness: float, diameter: float) -> float:
    ...
    turbulent = 0.25 / (
        math.log10(roughness / (3.7 * diameter) + 5.74 / reynolds ** 0.9) ** 2
    )
    if reynolds < 4000.0:
...
def darcy_weisbach_r(
    flow: float, length: float, diameter: float, roughness: float
) -> float:
...
def resistance(model: str, flow: float, length: float, diameter: float,
               coeff: float):

...
def build_system():
    ...
    net.add_venturi(
        VenturiLink(
            ...
            venturi=Venturi(a=4.0e6, injection_rate=2e-6, concentration=50.0),
        )
    )
    ...
    for i in range(1, 21):
        nid = f"E{i}"
        net.add_junction(Junction(id=nid, elevation=0.0, emitter=Emitter(k=k_em, x=x_em)))
        net.add_pipe(
            Pipe(id=f"L{i}", start=prev, end=nid, length=0.5, diameter=0.016, coeff=150, model="HW")
        )
        prev = nid
    return net
...
if __name__ == "__main__":
    ...
    print(f"FAO-56: ET0 = {et0:.2f} mm/day,  emitter design flow = " f"{q_design*3.6e6:.2f} L/h\n")

    net = build_system()
    result = solve(net)
    print(report(net, result))

    out = plot_lateral_profile(
        net, result, path=os.path.join(os.path.dirname(__file__), "lateral_profile.png")
    )
    print(f"\nProfile plot: {out}")

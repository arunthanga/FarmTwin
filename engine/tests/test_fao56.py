...
def test_et0_monotonic_in_radiation():
    base = dict(t_mean=29.0, t_min=23.0, t_max=35.0, rh_mean=60.0,
                wind_2m=1.8, elevation=120.0)
    low = fao56.et0_penman_monteith(rn=8.0, **base)
    high = fao56.et0_penman_monteith(rn=20.0, **base)
    assert high > low
    print("PASS test_et0_monotonic_in_radiation ", round(low, 2), "<",
          round(high, 2))
...
def test_water_balance_triggers_irrigation():
    ...
    print("PASS test_water_balance  TAW =", round(info["TAW"], 1),
          "RAW =", round(info["RAW"], 1), "Dr =", round(info["Dr"], 1))


def test_emitter_design_flow_positive():
    q = fao56.emitter_design_flow(
        net_irrigation_mm_per_day=5.0, area_per_emitter_m2=0.3,

...
def test_single_pipe_headloss():
    ...
    net.add_pipe(
        Pipe(id="P1", start="R", end="J", length=100.0, diameter=0.1, coeff=130.0, model="HW")
    )
    res = solve(net)
...
def test_two_reservoirs():
    ...
    net.add_pipe(Pipe(id="A", start="R1", end="J", length=500.0, diameter=0.2, coeff=120.0))
    net.add_pipe(Pipe(id="B", start="J", end="R2", length=500.0, diameter=0.2, coeff=120.0))
    res = solve(net)
    assert res.converged
    ...
    print(
        "PASS test_two_reservoirs  H_J =",
        round(res.heads["J"], 4),
        " Q =",
        round(res.flows["A"] * 3600, 3),
        "m3/h",
    )
...
def test_emitter_consistency():
    ...
    k, x = 3.51e-7, 0.5
    net.add_junction(Junction(id="E1", elevation=0.0, emitter=Emitter(k=k, x=x)))
    net.add_pipe(Pipe(id="P", start="R", end="E1", length=1.0, diameter=0.016, coeff=150.0))
    res = solve(net)
    assert res.converged
    p = res.pressures["E1"]
    q_emit = k * p**x
    ...
    assert 0 < p < 20.0
    print(
        "PASS test_emitter_consistency  P =", round(p, 3), "m  q =", round(q_emit * 3.6e6, 3), "L/h"
    )
...
def test_pump_head_gain():
    ...
    net.add_pipe(Pipe(id="P", start="OUT", end="J", length=50.0, diameter=0.05, coeff=140.0))
    res = solve(net)
    assert res.converged
    q = res.flows["PUMP"]
...
def test_minor_losses_increase_drop():
    ...
    print(
        "PASS test_minor_losses  P_plain =", round(p_plain, 3), "  P_fitted =", round(p_fitted, 3)
    )
